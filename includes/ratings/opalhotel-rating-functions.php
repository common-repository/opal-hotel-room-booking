...
if ( ! function_exists( 'opalhotel_print_hotel_star' ) ) {

	function opalhotel_print_hotel_star( $star = 5, $full = true ) {
		$count = 5;
        if ( ! $full )
            $count = $star;

        $html = array();
        $html[] = '<ul class="opalhotel-hotel-stars">';
        for ( $i = 1; $i <= $count; $i++ ) {
...
if ( ! function_exists( 'opalhotel_print_rating' ) ) {
    ...
    function opalhotel_print_rating( $star = 5, $full = true ) {
        ...
        $html = array();
        $html[] = '<ul class="opalhotel-review-stars">';
        for ( $i = 1; $i <= $count; $i++ ) {
            if ( ( $i <= $star ) || ( ( $i - $star ) <= 0.25 && ( $i - $star ) > 0 ) ) {
                $html[] = '<li><i class="fa fa-star"></i></li>';
            } else if( ( $i - $star ) < 0.75 && ( $i - $star ) >= 0.25 ){
...
if ( ! function_exists( 'opalhotel_get_average_rating' ) ) {
	...
	function opalhotel_get_average_rating( $post_id = null, $rating_item_id = null, $calculate = false ) {
		...
		return apply_filters( 'opalhotel_averger_rating_hotel', number_format( floatval( $average ), 2 ), $post_id );
	}
}
